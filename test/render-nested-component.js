...
describe("oc-client : renderNestedComponent", function() {
  ...
  describe("when rendering component does not succeed", function() {
    var $component, renderingError, renderingFailedEventData;

    beforeEach(function(done) {
      $component = $(componentContainer);
      renderingFailedEventData = undefined;
      renderingError = "An error!";
      initialise($component, renderingError);
      oc.events.on("oc:rendering-failed", function(_, eventData) {
        renderingFailedEventData = eventData;
      });
      oc.renderNestedComponent($component, done);
    });
    ...
    it("should fire oc:rendering-failed event", function() {
      expect(renderingFailedEventData).toBeDefined();
      expect(renderingFailedEventData.originalError).toContain(renderingError);
      expect(renderingFailedEventData.data).toBeDefined();
    });
  });
});
